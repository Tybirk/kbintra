...
def test_member_exports_csv(du, user, member):
    """æøå must survive the round trip, and the BOM is the only hint a CSV can give
    the reader about its encoding — so assert on the bytes, not just the text."""
    ...
    assert body.splitlines()[0] == "Nr.;Dato;Kategori;Beskrivelse;Hvor;Navn;Hus;Status;Afsluttet"
    # A description spans lines and carries commas; it has to come back quoted,
    # or every row after it lands one column off.
    assert '"Revne mellem bordplade og væg,\nvirker dårligt"' in body


@pytest.mark.django_db
def test_export_neutralizes_formula(du, user, member):
    """A pasted =SUM(...) is text, not something the sheet evaluates on open."""
    _report(du, user, description="=SUM(A1:A9) i brusekabinen")

    resp = _client(member).get("/api/reports/export/?subgroup=driftsudvalget")
    assert resp.status_code == 200
    assert "'=SUM(A1:A9) i brusekabinen" in resp.content.decode("utf-8-sig")

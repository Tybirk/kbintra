...
describe('AppNavbar', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(messagingApi.getUnreadCount).mockResolvedValue({ unread_count: 0 });
  ...
  it('should show unread messages badge', async () => {
    vi.mocked(messagingApi.getUnreadCount).mockResolvedValue({ unread_count: 5 });
  ...
  it('should show 9+ for more than 9 unread messages', async () => {
    vi.mocked(messagingApi.getUnreadCount).mockResolvedValue({ unread_count: 15 });

...
const TEXT_EXTENSIONS = ['txt', 'md', 'json', 'xml', 'csv', 'log', 'ini', 'yaml', 'yml'];
...
export function FilePreviewModal({ file, opened, onClose }: FilePreviewModalProps) {
      ...
      case 'word':
        return (
          <Stack align="center" gap="lg" py="xl">
            ...
            <Text size="lg" fw={500}>{file.name}</Text>
            <Text c="dimmed" ta="center">
              Word-dokumenter kan ikke vises direkte i browseren.
              <br />
              Download filen for at åbne den i Microsoft Word eller en kompatibel applikation.
            </Text>
            <Button
              leftSection={<IconDownload size={16} />}
              onClick={handleDownload}
        ...
        return (
          <Stack align="center" gap="lg" py="xl">
            ...
            <Text c="dimmed" ta="center">
              ...
              <br />
              Download filen for at åbne den i Microsoft PowerPoint eller en kompatibel applikation.
            </Text>
            <Button
              leftSection={<IconDownload size={16} />}
              onClick={handleDownload}
...
export function ImageThumbnail({ file, size = 48, onClick }: ImageThumbnailProps) {
  const fileType = getFileType(file.name);

  if (fileType !== 'image') {
    const Icon = getFileIcon(file.name);
    return <Icon size={size} color={`var(--mantine-color-${getFileTypeColor(file.name)}-6)`} />;
  }

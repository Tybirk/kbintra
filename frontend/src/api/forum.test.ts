...
describe('forumApi', () => {
  ...
  describe('Subgroups', () => {
    it('should fetch subgroups', async () => {
      ...
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockSubgroups } });
    ...
    it('should subscribe to subgroup', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: { detail: 'Subscribed' } });

      const result = await forumApi.subscribe('test-group');

      expect(apiClient.post).toHaveBeenCalledWith('/forum/subgroups/test-group/subscribe/');
      expect(result).toEqual({ detail: 'Subscribed' });
    });

    it('should unsubscribe from subgroup', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: { detail: 'Unsubscribed' } });

      const result = await forumApi.unsubscribe('test-group');

      expect(apiClient.post).toHaveBeenCalledWith('/forum/subgroups/test-group/unsubscribe/');
      expect(result).toEqual({ detail: 'Unsubscribed' });
    });

    it('should fetch subscriptions', async () => {
      const mockSubs = [{ id: 1, subgroup: 'test-group' }];
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockSubs } });
  ...
  describe('Threads', () => {
    it('should fetch threads for subgroup', async () => {
      ...
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockThreads } });

      const result = await forumApi.getThreads('test-group');

      expect(apiClient.get).toHaveBeenCalledWith('/forum/subgroups/test-group/threads/');
    ...
    it('should create thread', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/forum/subgroups/test-group/threads/', {
        title: 'New Thread',
        content: 'Thread content',
      });
      expect(result).toEqual(mockThread);
  ...
  describe('Posts', () => {
    it('should fetch posts for thread', async () => {
      ...
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockPosts } });
    ...
    it('should create post', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/forum/threads/1/posts/', { content: 'New Post' });
    ...
    it('should update post', async () => {
      ...
      expect(apiClient.patch).toHaveBeenCalledWith('/forum/posts/1/', { content: 'Updated Post' });
      expect(result).toEqual(mockPost);
    });
  ...
  describe('Folders', () => {
    it('should fetch root folders', async () => {
      ...
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockFolders } });

      const result = await forumApi.getFolders('test-group');

      expect(apiClient.get).toHaveBeenCalledWith('/forum/subgroups/test-group/folders/', {
        params: {},
      });
    ...
    it('should create folder', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/forum/subgroups/test-group/folders/', {
        name: 'New Folder',
        parent: null,
      });
      expect(result).toEqual(mockFolder);
  ...
  describe('Files', () => {
    it('should fetch files in folder', async () => {
      ...
      vi.mocked(apiClient.get).mockResolvedValue({ data: { results: mockFiles } });
    ...
    it('should fetch root files', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/forum/subgroups/test-group/files/');
      expect(result).toEqual(mockFiles);
    });
    ...
    it('should move file', async () => {
      vi.mocked(apiClient.patch).mockResolvedValue({ data: { detail: 'File moved' } });
